...
async function upload() {
  ...
  status.classList.remove('visible');  // Приховуємо статус
  status.innerHTML = '';                // Очищаємо текст
  ...
  try {
    ...
    if (text === 'OK') {
      status.innerHTML = `✅ Завантажено успішно!`;
    } else {
      status.innerHTML = `❌ Помилка: ${text}`;
    }

    setTimeout(() => status.classList.add('visible'), 50); // Плавно показуємо

  } catch (e) {
    loader.style.display = 'none';
    status.innerHTML = `⚠️ Помилка: ${e.message}`;
    setTimeout(() => status.classList.add('visible'), 50);
  }
}
