...
async function upload() {
  ...
  try {
    ...
    if (text === 'OK') {
      status.innerHTML = `<div style="color: #28a745; font-size: 18px;">✅ Upload successful</div>`;
    } else {
      status.innerHTML = `<div style="color: red; font-size: 16px;">❌ Upload failed: ${text}</div>`;
    }
  } catch (error) {
    loader.style.display = 'none';
    status.innerHTML = `<div style="color: red; font-size: 16px;">⚠️ Error: ${error.message}</div>`;
  }
}
